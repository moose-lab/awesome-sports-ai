  ...
  return `<article class="session-card">
    <header class="session-header">
      ...
      <div class="meta-grid">
        ...
        <span>${escapeHtml(plan.equipmentLine)}</span>
        <span>${escapeHtml(plan.experienceLine)}</span>
        <span>${escapeHtml(plan.limiterLine)}</span>
    ...
    <section class="goal-grid">
      ...
      <div>
        <h2>Intended Stimulus / 目标刺激</h2>
        <p>${escapeHtml(plan.intendedStimulus.en)}</p>
        <p lang="zh-CN">${escapeHtml(plan.intendedStimulus.zh)}</p>
      </div>
    </section>

    <section class="standard">
      <h2>Mechanics, Consistency, Intensity / 动作机制、稳定性、强度</h2>
      <p>${escapeHtml(plan.progressionStandard)}</p>
    </section>
    ...
    <section class="constraints">
      <h2>Scaling Path / Scale 路径</h2>
      ${renderList(plan.scalingPath)}
    </section>
    ...
    <section class="readiness">
      ...
      <div class="readiness-row">
        ...
        <p><strong>Yellow / 黄色</strong><br>Reduce load, reps, or rounds by 20-30%. / 重量、次数或轮数减少 20-30%。</p>
...
<head>
  ...
  <title>CrossFit Daily Training Checklist</title>
  <style>
    :root {
      color-scheme: light;
      --bg: #f5f5f2;
